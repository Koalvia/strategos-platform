...
@pytest.mark.unit
def test_bc_outage_leaves_the_scope_unrestricted():
    """``customersResources`` degrades to ``[]`` on failure, so nothing is hidden wrongly.

    The resolver cannot tell an outage from "no assignments", which is why the live
    client logs the failure — asserted here as the documented consequence.
    """
